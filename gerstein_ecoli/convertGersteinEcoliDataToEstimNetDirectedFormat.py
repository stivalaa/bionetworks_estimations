...
##############################################################################
#
# File:    convertGersteinEcoliDataToEstimNetDirectdFormat.py 
# Author:  Alex Stivala
# Created: April 2019
#
##############################################################################

"""Load the Gerstein lab ecoli regulatory network data in adjacency list
...
The ecoli_idmap.txt has two columns, first is original name, second is 
...
http://archive.gersteinlab.org/proj/Hierarchy_Rewiring/PNAS_hier/ERN.txt
...
def main():
    ...
    g = load_from_adjacency_lists("ERN.txt")
    ...
    g.write_pajek('ecoli_arclist.txt')

    f = open('ecoli_binattr.txt', 'w')
    f.write('self\n')
    f.write('\n'.join([('1' if x else '0') for x in g.vs['self']]))
    f.write('\n')
    f.close()

    f = open('ecoli_idmap.txt', 'w')
