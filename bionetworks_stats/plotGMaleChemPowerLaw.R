...
#
# Usage: Rscript plotGMaleChemPowerLaw.R 
#
...
descrs <- c(
    expression(italic('C. elegans')~~'neural')
    )
...
if (length(args) != 0) {
  cat("Usage: plotGMaleChemPowerLaw\n")
  quit(save="no")
}
datadir <- args[1]

outfile <- "GMaleChem_powerlaw.eps"
