#!/usr/bin/Rscript
#
# File:    taperedEstimateEcoli1Model1.R
# Author:  Alex Stivala
# Created: May 2023
#
...
# Usage:
# 
# Rscript taperedEstimateEcoli1Model1.R 
#
# Output files (WARNING overwritten)
#   ecoli1_tapered_model1.txt
#   ecoli1_tapered_model1_mcmcdiagnostics.eps
#   gof_ecoli11_tapered_model1.eps
#   ecoli1_tapered_model1.RData
#
...
system.time(tapered_model1  <- ergm.tapered(ecoli1 ~ edges + gwidegree(2, fixed=TRUE) + gwodegree(0, fixed=TRUE) + ttriple + twopath, taper.terms = "dependent", fixed = FALSE))

print(tapered_model1)
summary(tapered_model1, extended = TRUE)

postscript('ecoli_tapered_model1_mcmcdiagnostics.eps')
mcmc.diagnostics(tapered_model1)
dev.off()

sink('ecoli_tapered_model1.txt')
...
postscript('ecoli_tapered_model1_gof.eps')
par(mfrow=c(4,2))
