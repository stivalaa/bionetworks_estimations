#!/usr/bin/Rscript
#
# File:    taperedEstimateFlyMedullaNamedModel3.R
# Author:  Alex Stivala
# Created: June 2023
#
...
# Usage:
# 
# Rscript taperedEstimateFlyMedullaNamedModel3.R 
#
# Output files (WARNING overwritten)
#   fly_medulla_named_tapered_model3.txt
#   fly_medulla_named_tapered_model3_mcmcdiagnostics.eps
#   gof_fly_medulla_named1_tapered_model3.eps
#   fly_medulla_named_tapered_model3.RData
#
...
system.time(tapered_model3  <- ergm.tapered(gn ~ edges + gwidegree(0.2, fixed = TRUE) + gwodegree(0.2, fixed = TRUE) + ttriple + twopath + edgecov(distmatrix_named) + mutual, taper.terms = "dependent", fixed = FALSE))


print(tapered_model3)
summary(tapered_model3, extended = TRUE)

postscript('fly_medulla_named_tapered_model3_mcmcdiagnostics.eps')
mcmc.diagnostics(tapered_model3)
dev.off()

sink('fly_medulla_named_tapered_model3.txt')
print( summary(tapered_model3, extended = FALSE) )
sink()

save(gn, tapered_model3, file = "fly_medulla_named_tapered_model3.RData")

system.time( tapered_model3_gof <-  gof(tapered_model3, GOF =  ~ idegree + odegree + distance + espartners + dspartners + triadcensus + model) )
print(tapered_model3_gof)
postscript('fly_medulla_named_tapered_model3_gof.eps')
par(mfrow=c(4,2))
plot( tapered_model3_gof, plotlogodds=TRUE)
dev.off()
