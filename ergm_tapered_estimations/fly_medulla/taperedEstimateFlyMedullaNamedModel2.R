#!/usr/bin/Rscript
#
# File:    taperedEstimateFlyMedullaNamedModel2.R
# Author:  Alex Stivala
# Created: June 2023
#
...
# Usage:
# 
# Rscript taperedEstimateFlyMedullaNamedModel2.R 
#
# Output files (WARNING overwritten)
#   fly_medulla_named_tapered_model2.txt
#   fly_medulla_named_tapered_model2_mcmcdiagnostics.eps
#   gof_fly_medulla_named1_tapered_model2.eps
#   fly_medulla_named_tapered_model2.RData
#
...
system.time(tapered_model2  <- ergm.tapered(gn ~ edges + gwidegree(0.2, fixed = TRUE) + gwodegree(0.2, fixed = TRUE) + ttriple + twopath + edgecov(distmatrix_named), taper.terms = "dependent", fixed = FALSE))


print(tapered_model2)
summary(tapered_model2, extended = TRUE)

postscript('fly_medulla_named_tapered_model2_mcmcdiagnostics.eps')
mcmc.diagnostics(tapered_model2)
dev.off()

sink('fly_medulla_named_tapered_model2.txt')
print( summary(tapered_model2, extended = FALSE) )
sink()

save(gn, tapered_model2, file = "fly_medulla_named_tapered_model2.RData")
...
postscript('fly_medulla_named_tapered_model2_gof.eps')
par(mfrow=c(4,2))
