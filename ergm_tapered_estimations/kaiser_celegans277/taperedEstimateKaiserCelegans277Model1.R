#!/usr/bin/env Rscript
###
### File:    taperedEstimateKaiserCelegans277Model1.R
### Author:  Alex Stivala
...
###
### Usage: taperedEstimateKaiserCelegans277Model1.R
###
###
### Model and GoF output is to stdout, also writes output files to cwd:
###   'celegans277_tapered_model1.txt'
###   'celegans277_tapered_model1_mcmcdiagnostics.eps'
###   'celegans277_tapered_model1_gof.eps'
###   'model1.RData'
...
postscript('celegans277_tapered_model1_mcmcdiagnostics.eps')
mcmc.diagnostics(model1)
dev.off()

sink('celegans277_tapered_model1.txt')
print( summary(model1) )
sink()

save(gn, mode1, file = "model1.RData")

system.time( model1_gof <- gof(model1  ~ idegree + odegree + distance + espartners + dspartners + triadcensus + model)  )
print(model1_gof)
postscript('celegans277_tapered_model1_gof.eps')
par(mfrow=c(4,2))
