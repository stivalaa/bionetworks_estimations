#!/bin/sh
#
# File:    reorderStatnetTextable.sh
# Author:  Alex Stivala
# Created: December 2017
#
#
# Read output of statnetEstimation2textableMultiModels.sh and permute the table
# rows according to specified order, in order to present the rows in an order we determine
# as best for presentation.
# 
# Usage: reorderStatnetTextable.sh <intable> <permutation>
#
# where <intable> is the input tex file and
#       <permutation> is a permutation of 0..n-1 e.g. 2 0 1
#
# E.g.:
#   reorderStatnetTextable.sh  ecoli_estimations.tex  1 5 4 2 3 0 9 10 7 6
#
# Output is to stdout
#
...
# assume format of input latex table from statnetEstimation2textableMultiModels.sh
# 8 header rows and 4 trailer rows
headrows=8
tailrows=4
