...
##############################################################################
#
# convertMatrixBinAttrToSMNetFormat.py - matrix+binary attr to SMNet format
#
#
# File:    convertMatrixBinAttrToSMNetFormat.py
# Author:  Alex Stivala
...
"""
Convert (older version) PNet network (adjacency matrix) 
and binary attributes to
...
works there after upgrade so have to use Python scripts and C program instead.

Usage:
    converEdgeListBinAttrToSMNetFormat.py matrixfile binattrfile
    
 Output files are (WARNING: overwritten in cwd):

    sampledesc.tx	- referred to in setting.txt, names other files
...
def usage(progname):
    ...
    """
    sys.stderr.write("usage: " + progname + " matrixfile binattrfile\n")
    sys.exit(1)
...
def main():
    ...
    edgelisttempfile = tempfile.NamedTemporaryFile()
    os.system("convertMatrixToEdgeList " + matrixfile + " " + edgelisttempfile.name)

    edgelist = open(edgelisttempfile.name).read()
    ...
    #os.unlink(edgelisttempfile.name) alrady automatically deleted
