#!/usr/bin/Rscript
##
## File:    plotMfinderSimFit.R
## Author:  Alex Stivala
## Created: November 2018
##
##
## Based on plotEstimNetDirectedSimFit.R but just changed input 
## network file formats to mfinder format instead:
...
## addition to the dzA value plot from plotMfinderResults.R
...
##
## Usage: Rscript plotMfinderSimFit netfilename simNetFilePrefix
##  netfilename is the mfinder format observed graph (the input file
##     for Mfinder)
##  simNetFilePreifx is the prefix of the randomized network filenames
##    this files have _RAND_x.txt appended by Mfinder, where x
##    is sequence number.
##
...
##
## Example:
## Rscript plotMfinderSimFit ecoli1.txt ecoli1_RAND
##  which will use input files ecoli1_RAND_1.txt etc. and write
##   to ecoli1_RAND.pdf
##
...
##
##
## Note that Mfinder can handle very large graphs, but R and igraph
...
##
## Read network in mfinder file format
##
## 3 columns: first to are soure and dest nodes (respectively),
## intergers from 1..N; 3rd is edge weight (unused always 1)
##
## Parameters: 
##      filename - name of mfinder network file to read
##      directed  - optional default TRUE; must be TRUE
##
##  Return value;
##      igraph directed graph object
##
read_mfinder_net_file <- function(filename, directed=TRUE) {
  stopifnot(directed)
  edgelist <- read.table(filename, header=FALSE)
  return(graph_from_edgelist(as.matrix(edgelist[c(1,2)]), directed=directed))
}
...
if (length(args) != 2) {
  cat("Usage: Rscript plotMfinderSimFit.R netfilename simNetFilePrefix\n")
  quit(save="no")
}
netfilename <- args[1]
