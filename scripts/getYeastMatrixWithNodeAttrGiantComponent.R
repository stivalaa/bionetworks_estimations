#!/usr/bin/Rscript
#
# File:    getYeastMatrixWithAttrGiantComponent.R
# Author:  Alex Stivala
# Created: November 2016
#
#
# Get yeast protein interaction network from Nexus and wrte files for
# SPNet estimation with node attributes.
...
#
# This version gets jsut the giant compeontn of the network.
#
# Usage:
# 
# Rscript getYeastMatrixWithAttrGiantComponent.R 
#
# Output files are (WARNING: overwritten in cwd):
#
#    yeast_giantcomponent_matrix.txt    - giant component adjacency matrix
#    yeast_giantcomponent_categorical_attributes.txt - categorical node attrs.
#
...
if (length(args) != 0) {
  cat("Usage: getYeastMatrixWithAttr.R\n")
  quit(save="no")
}
...
write_graph_file(network_filename, g, write_header=FALSE)

f = "yeast_giantcomponent_categorical_attributes.txt"
