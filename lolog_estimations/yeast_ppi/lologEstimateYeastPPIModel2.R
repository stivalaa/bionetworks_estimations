#!/usr/bin/Rscript
#
# File:    lologEstimateYeastPPI1Model2.R
# Author:  Alex Stivala
# Created: June 2021
#
...
# Usage:
# 
# Rscript lologEstimateYeastPPI1Model2.R 
#
# Output files (WARNING overwritten)
#   yeast_ppi_lolog_model2.txt
#   yeast_ppi_lolog_model2.pdf
#   gof_yeast_ppi1_lolog_model2_X.pdf
...
system.time( yeast_ppi_lolog_model2 <- lolog(yeast_ppi ~ edges + twoPath + triangles + gwdegree(.2)) )
#system.time( yeast_ppi_lolog_model2 <- lolog(yeast_ppi ~ edges + twoPath + triangles + gwdegree(.2) +  nodeMatch('class')) )

summary(yeast_ppi_lolog_model2)
sink('yeast_ppi_lolog_model2.txt')
summary(yeast_ppi_lolog_model2)
sink()

pdf('yeast_ppi_lolog_model2.pdf')
plot(yeast_ppi_lolog_model2)
dev.off()

system.time( gof_yeast_ppi_lolog_model2_degree <- gofit(yeast_ppi_lolog_model2, yeast_ppi ~ degree(0:maxdeg)) )
system.time( gof_yeast_ppi_lolog_model2_esp <- gofit(yeast_ppi_lolog_model2, yeast_ppi ~ esp(0:25)) )
system.time( gof_yeast_ppi_lolog_model2_edges <- gofit(yeast_ppi_lolog_model2, yeast_ppi ~ edges + mutual) )

# Does not work: par(mfrow=c(2,2))
# so have to have separate file for each plot
pdf('gof_yeast_ppi_lolog_model2_degree.pdf')
plot(gof_yeast_ppi_lolog_model2_degree)
dev.off()
pdf('gof_yeast_ppi_lolog_model2_esp.pdf')
plot(gof_yeast_ppi_lolog_model2_esp)
dev.off()
pdf('gof_yeast_ppi_lolog_model2_edges.pdf')
plot(gof_yeast_ppi_lolog_model2_edges, type='box')
dev.off()
