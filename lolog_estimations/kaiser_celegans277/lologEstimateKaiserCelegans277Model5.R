#!/usr/bin/env Rscript
###
### File:    lologEstimateKaiserCelegans277Model5.R
### Author:  Alex Stivala
...
###
### Usage: lologEstimateKaiserCelegans277Model5.R
###
### Model and GoF output is to stdout, also writes output files to cwd:
###   celegans277_lolog_model5.txt
###   celegans277_lolog_model5.pdf
###   gof_celegans2771_lolog_model5_X.pdf
...
system.time( celegans277_lolog_model5 <- lolog(gn ~ edges + twoPath + triangles + gwdegree(.2, direction='in') + gwdegree(.2, direction='out') + edgeCov(celegans277_distmatrix) | birthtime) )



summary(celegans277_lolog_model5)
sink('celegans277_lolog_model5.txt')
summary(celegans277_lolog_model5)
sink()

pdf('celegans277_lolog_model5.pdf')
plot(celegans277_lolog_model5)
dev.off()

system.time( gof_celegans277_lolog_model5_indegree <- gofit(celegans277_lolog_model5, gn ~ degree(0:maxindeg, direction='in')))
system.time( gof_celegans277_lolog_model5_outdegree <- gofit(celegans277_lolog_model5, gn ~ degree(0:maxoutdeg, direction='out')))
system.time( gof_celegans277_lolog_model5_esp <- gofit(celegans277_lolog_model5, gn ~ esp(0:25)) )
system.time( gof_celegans277_lolog_model5_edges <- gofit(celegans277_lolog_model5, gn ~ edges + mutual) )

# Does not work: par(mfrow=c(2,2))
# so have to have separate file for each plot
pdf('gof_celegans277_lolog_model5_indegree.pdf')
plot(gof_celegans277_lolog_model5_indegree)
dev.off()
pdf('gof_celegans277_lolog_model5_outdegree.pdf')
plot(gof_celegans277_lolog_model5_outdegree)
dev.off()
pdf('gof_celegans277_lolog_model5_esp.pdf')
plot(gof_celegans277_lolog_model5_esp)
dev.off()
pdf('gof_celegans277_lolog_model5_edges.pdf')
plot(gof_celegans277_lolog_model5_edges, type='box')
dev.off()
