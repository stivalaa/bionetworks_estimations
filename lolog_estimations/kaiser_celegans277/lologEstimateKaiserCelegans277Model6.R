#!/usr/bin/env Rscript
###
### File:    lologEstimateKaiserCelegans277Model6.R
### Author:  Alex Stivala
...
###
### Usage: lologEstimateKaiserCelegans277Model6.R
###
### Model and GoF output is to stdout, also writes output files to cwd:
###   celegans277_lolog_model6.txt
###   celegans277_lolog_model6.pdf
###   gof_celegans2771_lolog_model6_X.pdf
...
# (took 1.8 hours on lenovo pc)
system.time( celegans277_lolog_model6 <- lolog(gn ~ edges + twoPath + triangles + gwdegree(.2, direction='in') + gwdegree(.2, direction='out') +  edgeCov(celegans277_distmatrix) + nodeCov('birthtime') | birthtime) )

## No diff() in lolog


summary(celegans277_lolog_model6)
sink('celegans277_lolog_model6.txt')
summary(celegans277_lolog_model6)
sink()

pdf('celegans277_lolog_model6.pdf')
plot(celegans277_lolog_model6)
dev.off()

system.time( gof_celegans277_lolog_model6_indegree <- gofit(celegans277_lolog_model6, gn ~ degree(0:maxindeg, direction='in')))
system.time( gof_celegans277_lolog_model6_outdegree <- gofit(celegans277_lolog_model6, gn ~ degree(0:maxoutdeg, direction='out')))
system.time( gof_celegans277_lolog_model6_esp <- gofit(celegans277_lolog_model6, gn ~ esp(0:25)) )
system.time( gof_celegans277_lolog_model6_edges <- gofit(celegans277_lolog_model6, gn ~ edges + mutual) )

# Does not work: par(mfrow=c(2,2))
# so have to have separate file for each plot
pdf('gof_celegans277_lolog_model6_indegree.pdf')
plot(gof_celegans277_lolog_model6_indegree)
dev.off()
pdf('gof_celegans277_lolog_model6_outdegree.pdf')
plot(gof_celegans277_lolog_model6_outdegree)
dev.off()
pdf('gof_celegans277_lolog_model6_esp.pdf')
plot(gof_celegans277_lolog_model6_esp)
dev.off()
pdf('gof_celegans277_lolog_model6_edges.pdf')
plot(gof_celegans277_lolog_model6_edges, type='box')
dev.off()
