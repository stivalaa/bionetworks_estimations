#!/usr/bin/env Rscript
###
### File:    lologEstimateKaiserCelegans277Model7.R
### Author:  Alex Stivala
...
###
### Usage: lologEstimateKaiserCelegans277Model7.R
###
### Model and GoF output is to stdout, also writes output files to cwd:
###   celegans277_lolog_model7.txt
###   celegans277_lolog_model7.pdf
###   gof_celegans2771_lolog_model7_X.pdf
...
# took approx. 10 minutes on lenovo pc
system.time( celegans277_lolog_model7 <- lolog(gn ~ edges + twoPath + triangles + gwdegree(.2, direction='in') + gwdegree(.2, direction='out') +  edgeCov(celegans277_distmatrix) + nodeCov('birthtime')) )

## No diff() in lolog


summary(celegans277_lolog_model7)
sink('celegans277_lolog_model7.txt')
summary(celegans277_lolog_model7)
sink()

pdf('celegans277_lolog_model7.pdf')
plot(celegans277_lolog_model7)
dev.off()

system.time( gof_celegans277_lolog_model7_indegree <- gofit(celegans277_lolog_model7, gn ~ degree(0:maxindeg, direction='in')))
system.time( gof_celegans277_lolog_model7_outdegree <- gofit(celegans277_lolog_model7, gn ~ degree(0:maxoutdeg, direction='out')))
system.time( gof_celegans277_lolog_model7_esp <- gofit(celegans277_lolog_model7, gn ~ esp(0:25)) )
system.time( gof_celegans277_lolog_model7_edges <- gofit(celegans277_lolog_model7, gn ~ edges + mutual) )

# Does not work: par(mfrow=c(2,2))
# so have to have separate file for each plot
pdf('gof_celegans277_lolog_model7_indegree.pdf')
plot(gof_celegans277_lolog_model7_indegree)
dev.off()
pdf('gof_celegans277_lolog_model7_outdegree.pdf')
plot(gof_celegans277_lolog_model7_outdegree)
dev.off()
pdf('gof_celegans277_lolog_model7_esp.pdf')
plot(gof_celegans277_lolog_model7_esp)
dev.off()
pdf('gof_celegans277_lolog_model7_edges.pdf')
plot(gof_celegans277_lolog_model7_edges, type='box')
dev.off()
